...
export default defineComponent({
	...
	setup() {
		...
		const currentPost = ref<PostProps>(null)
		onMounted(() => {
			getCurrentPost()
		})
		//获取单篇文章信息
		const getCurrentPost = async () => {
			...
			currentPost.value = await getPost(currentId)
		}
		...
		const hideAndDelete = async () => {
			...
			try {
				const rawData: PostProps = await delPost(currentId)
				createMessage('删除成功，2秒后跳转到专栏首页', 'success', 2000)
				setTimeout(() => {
					router.push({ name: 'Detail', params: { id: rawData.column } })
				}, 2000)
			} catch (err) {
				console.log(err)
			}
